 ...
 */
public class RepositoryEditPanel extends JDialog implements UIConstants, DataAccessConstants {
        ...
        }
        {
            ...
            }
            {
                ...
                }
                {
                    ...
                    protocolePanel.add(comboBoxProtocole);
                    ComboBoxModel comboBoxProtocoleModel = new DefaultComboBoxModel(new String[] {
                            Protocole.FTP.getDescription(), Protocole.HTTP.getDescription() });
    ...
    private void buttonImportPerformed() {
        ...
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (invalid) {
                    JOptionPane.showMessageDialog(facade.getMainPanel(), "Url must ends with "
                            + "\"" + DataAccessConstants.AUTOCONFIG + "\"" + ".", "Warning",
                ...
                try {
                    AutoConfigDTO autoConfigDTO = connexion.importAutoConfig(url);
                    if (autoConfigDTO != null) {
                        // Init UI fields
                        labelConnection.setText("Connection success!");
                        labelConnection.setFont(new Font("Tohama", Font.ITALIC, 11));
                        labelConnection.setForeground(new Color(45, 125, 45));
                        textFieldRepositoryName.setText(autoConfigDTO.getRepositoryName());
                        Protocole protocole = autoConfigDTO.getProtocoleDTO().getProtocole();
                        comboBoxProtocole.setSelectedItem(protocole.getDescription());
                        textFieldHost.setText(protocole.getPrompt()
                                + autoConfigDTO.getProtocoleDTO().getUrl());
                        textFieldLogin.setText(autoConfigDTO.getProtocoleDTO().getLogin());
                        textFieldPort.setText(autoConfigDTO.getProtocoleDTO().getPort());
                        passwordField.setText(autoConfigDTO.getProtocoleDTO().getPassword());
                    ...
                    JOptionPane.showMessageDialog(facade.getMainPanel(), e1.getMessage(), "Error!",
                            JOptionPane.ERROR_MESSAGE);
                }
    ...
    private void buttonOKPerformed() {
        ...
        if (name.contains("/") || name.contains("\\")) {
            JOptionPane.showMessageDialog(this, "Repository must not contains characters / or \\",
                    "Warning", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        try {
            ...
            repositoryService.createRepository(name, url, port, login, pass, protocole);
            ...
            JOptionPane.showMessageDialog(this,
                    "An error occured. \n Failded to write repository.", "Error",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    private void comboBoxEncryptionPerformed() {
        JOptionPane.showMessageDialog(this, "Not implemented yet.", "Repository",
