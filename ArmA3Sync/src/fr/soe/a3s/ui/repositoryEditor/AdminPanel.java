...
public class AdminPanel extends JPanel implements UIConstants {
    ...
    private JButton buttonSelectFTPfolderPath, buttonBuild, buttonCopyAutoConfigURL, buttonCheck;

    private final RepositoryService repositoryService = new RepositoryService();

    private String repositoryName;

    private JTextField textFieldftpSharedFolderLocation, textFieldAutoConfigURL;
    ...
    public AdminPanel(Facade facade) {
        ...
        repositoryAdministrationPanel.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(), "Administration"));
        ...
        }
        {
            ...
            locationPanel.add(textFieldftpSharedFolderLocation, BorderLayout.CENTER);
        ...
        vBox.add(Box.createVerticalStrut(5));
        {
            ...
            JLabel autoConfigLabelLocation = new JLabel("Repository auto-config url");
    ...
    public void init(String repositoryName) {
        ...
        try {
            ...
            if (repositoryDTO.getAutoConfigURL() != null) {
                textFieldAutoConfigURL.setText(repositoryDTO.getProtocoleDTO().getProtocole()
                        .getPrompt()
                        + repositoryDTO.getAutoConfigURL());
            }

            if (repositoryDTO.isOutOfSynk()) {
                labelStatusValue.setText(RepositoryStatus.OUTOFSYNC.getDescription());
    ...
    private void buttonSelectFTPfolderPathPerformed() {
            ...
            if (index != -1) {// There is a folder after the root
                ...
                if (!ftpFolderName.equalsIgnoreCase(folderName)) {
                    JOptionPane.showMessageDialog(facade.getMainPanel(),
                            "The selected FTP shared folder " + file.getName()
                    ...
                    textFieldftpSharedFolderLocation.setText(file.getAbsolutePath());
                }
            }
            else {
                textFieldftpSharedFolderLocation.setText(file.getAbsolutePath());
        ...
        try {
            repositoryService.setRepositoryPath(repositoryName, textFieldftpSharedFolderLocation
                    .getText().trim());
    ...
    private void buttonBuildPerformed() {
        ...
        if (path.isEmpty()) {
            JOptionPane.showMessageDialog(facade.getMainPanel(),
                    "Repository main folder location is empty!", "Build repository",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
        else if (!(new File(path).exists())) {
            JOptionPane.showMessageDialog(facade.getMainPanel(),
                    "Repository main folder does not exists!", "Build repository",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }

        RepositoryBuilder repositoryBuilder = new RepositoryBuilder(facade, repositoryName, path,
                this);
    ...
    private void buttonCheckPerformed() {
        ...
        if (path.isEmpty()) {
            JOptionPane.showMessageDialog(facade.getMainPanel(),
                    "Repository main folder location is empty!", "Check repository",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
        else if (!(new File(path).exists())) {
            JOptionPane.showMessageDialog(facade.getMainPanel(),
                    "Repository main folder does not exists!", "Check repository",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }

        RepositoryChecker checker = new RepositoryChecker(facade, repositoryName, path);
        checker.start();
    }
