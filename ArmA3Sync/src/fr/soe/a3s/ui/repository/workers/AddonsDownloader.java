...
public class AddonsDownloader extends Thread implements DataAccessConstants {
    ...
    public AddonsDownloader(Facade facade, String repositoryName, SyncTreeDirectoryDTO racine,
    ...
    public void run() {
        ...
        System.out.println("Starting downloading from repository: " + repositoryName);
        ...
        if (this.listFilesToUpdate.size() == 0 && listFilesToDelete.size() == 0) {
            ...
            JOptionPane.showMessageDialog(facade.getMainPanel(), "Nothing to download.",
        ...
        try {
            ...
            connexionService = ConnexionServiceFactory.getServiceForFilesSynchronization(
                    repositoryName, numberOfConnections);

        }
        catch (RepositoryException | CheckException e) {
            JOptionPane.showMessageDialog(facade.getMainPanel(), e.getMessage(), "Download",
        ...
        try {
            for (AbstractConnexionDAO connect : connexionService.getConnexionDAOs()) {
                ...
                connect.addObserverDownload(new ObserverDownload() {
                    ...
                    @Override
                    public void updateCancelTooManyTimeoutErrors(int value, List<Exception> errors) {
            ...
            for (SyncTreeNodeDTO node : listFilesToUpdate) {
                if (!node.getDownloadStatus().equals(DownloadStatus.DONE)) {
                    ...
                    if (leaf.getDownloadStatus().equals(DownloadStatus.DONE) && leaf.isCompressed()) {
                        File parentDirectory = new File(leaf.getDestinationPath());
                        File zipFile = new File(parentDirectory + "/" + leaf.getName()
                                + ZIP_EXTENSION);
                        if (zipFile.exists()) {
                            connexionService.getUnZipFlowProcessor().unZipAsynchronously(zipFile);
                        }
                    }
                }
            }
            ...
            if (e instanceof RepositoryException) {
                ...
                if (e instanceof IOException) {
                    ...
                    UnexpectedErrorDialog dialog = new UnexpectedErrorDialog(facade, "Download", e,
                            repositoryName);
                    dialog.show();
                }
    ...
    private synchronized void executeUpdateTotalSizeProgress(long value) {
        ...
        if (totalExpectedFilesSize != 0) {// division by 0!
            SwingUtilities.invokeLater(new Runnable() {
                ...
                public void run() {
                    ...
                    downloadPanel.getProgressBarDownloadAddons().setValue(pourcentage);
                }
            });
        }
        ...
        if (downloadedFilesSize != 0) {
            ...
            downloadPanel.getLabelRemainingTimeValue().setText(
                    UnitConverter.convertTime((long) (remaininTime * Math.pow(10, -9))));
        }
    }
    ...
    private synchronized void executeUpdateSingleSizeProgress(final long value,
        ...
        double endTime = System.nanoTime();
        double elapsedTime = endTime - startTime;
        long remainingFilesSize = totalExpectedFilesSize - incrementedFilesSize - value;
        long downloadedFilesSize = incrementedFilesSize + value - initialFilesSize;
        if (downloadedFilesSize != 0) {
            final long remainingTime = (long) ((remainingFilesSize * elapsedTime) / downloadedFilesSize);
            downloadPanel.getLabelRemainingTimeValue().setText(
                    UnitConverter.convertTime((long) (remainingTime * Math.pow(10, -9))));
        }
    }
    ...
    private synchronized void executeUpdateDownloadSpeed() {
        ...
        downloadPanel.getLabelSpeedValue().setText(UnitConverter.convertSpeed(speed));
    ...
    private synchronized void executeUpdateActiveConnections() {
        ...
        downloadPanel.getLabelActiveConnectionsValue().setText(Integer.toString(activeConnections));
    ...
    private void finish(String message, List<Exception> errors) {
        ...
        downloadPanel.getLabelSpeedValue().setText(UnitConverter.convertSpeed(0));
        downloadPanel.getLabelRemainingTimeValue().setText(UnitConverter.convertTime(0));

        /* Delete extra files */
        downloadPanel.getLabelDownloadStatus().setText("Deleting extra files...");
        deleteExtraFiles();
        ...
        if (errors == null) {
            ...
            downloadPanel.getLabelDownloadStatus().setForeground(DownloadPanel.GREEN);
            JOptionPane.showMessageDialog(facade.getMainPanel(), "Download is finished.",
                    "Download", JOptionPane.INFORMATION_MESSAGE);
        }
        else {
            downloadPanel.getLabelDownloadStatus().setText("Error!");
        ...
        if (errors == null) {
            if (tfarIsUpdated) {
                int response = JOptionPane.showConfirmDialog(facade.getMainPanel(),
                        "TFAR files have changed. Proceed with TFAR installer?", "TFAR installer",
        ...
        if (errors == null) {
            if (userconfigNode != null) {
                ...
                if (arma3Directory != null) {
                    if (!arma3Directory.getAbsolutePath().equals(defaultDownloadLocation)) {
                        int response = JOptionPane.showConfirmDialog(facade.getMainPanel(),
                                "Userconfig folder have changed." + "\n"
                                        + "Apply changes into ArmA 3 installation directory?"
                                        + "\n\n" + "Note: extra local content wont be deleted.",
                                "Userconfig", JOptionPane.OK_CANCEL_OPTION);
                        if (response == 0) {
                            String userconfigSourcePath = userconfigNode.getDestinationPath() + "/"
                            ...
                            if (!userconfigSourceDirectory.exists()) {
                                JOptionPane.showMessageDialog(facade.getMainPanel(),
                                        "File not found:" + userconfigSourcePath, "Userconfig",
                                        JOptionPane.ERROR_MESSAGE);
                            }
                            else {
                                ...
                                if (!userconfigTargetDirectory.exists()) {
                                    JOptionPane.showMessageDialog(facade.getMainPanel(),
                                            "Failed to create directory: " + userconfigTargetPath
                                                    + "\n"
                                                    + "Please checkout file access permissions.",
                                            "Userconfig", JOptionPane.ERROR_MESSAGE);
                                }
                                else {
                                    try {
                                        FileAccessMethods.copyDirectory(userconfigSourceDirectory,
    ...
    private synchronized void executeCancelTooManyErrors(int value, List<Exception> errors) {
    ...
    private synchronized void executeCancelTooManyTimeoutErrors(int value, List<Exception> errors) {
    ...
    private String generateReport(String message) {
        ...
        String serverConnectionInfo = "Server connection:" + "\n" + "- Average response time: "
                ...
                + "- Number of active connections used: " + maxActiveconnections;

        // Global File transfer:
        String savedSizeFileTransfer = UnitConverter.convertSize(totalDiskFilesSize
        ...
        String savedPartialSizeFileTransfer = UnitConverter.convertSize(totalUncompleteDiskFileSize
        ...
        String partialFileTransferInfo = "Partial file transfer:" + "\n"
                ...
                + savedPartialSizeFileTransfer + " (" + savedPartialSizeFileTransferFraction + "%)";

        // Compressed file transfer
        String savedCompressedSizeFileTransfer = UnitConverter
                .convertSize(totalUncompressedFilesSize - totalCompressedFilesSize);
        ...
        String compressionFileTransferInfo = "Compressed file transfer:" + "\n"
                ...
                + "- Downloaded data: " + UnitConverter.convertSize(totalCompressedFilesSize)
    ...
    private void isUserconfigParent(SyncTreeNodeDTO node) {
        ...
        if (!node.isLeaf()) {
            ...
            if (directory.getName().toLowerCase().equals("userconfig")) {
                if (directory.getParent().getName().equals(SyncTreeDirectoryDTO.RACINE)) {
                    ok = true;
                }
            }
        }
    ...
    private void getFiles(SyncTreeNodeDTO node) {
        ...
        if (!node.isLeaf()) {
            ...
            else if (syncTreeLeafDTO.isSelected() && syncTreeLeafDTO.isDeleted()) {
            ...
            if (node.isLeaf()) {
                ...
                if (leaf.getComplete() != 0) {
                    ...
                    totalUncompleteExpectedFileSize = totalUncompleteExpectedFileSize
                            + FileSizeComputer.computeExpectedSize(leaf);
                    totalUncompleteDiskFileSize = totalUncompleteDiskFileSize + leaf.getSize();
                }
                else if (leaf.isCompressed()) { // TotalCompressedFilesSize &
                                                // TotalUncompressedFilesSize
                    compressedFiles++;
                    totalCompressedFilesSize = totalCompressedFilesSize + leaf.getCompressedSize();
                    totalUncompressedFilesSize = totalUncompressedFilesSize + leaf.getSize();
                }
            }
        }
