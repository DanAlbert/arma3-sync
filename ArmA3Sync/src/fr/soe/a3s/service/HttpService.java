...
public class HttpService extends AbstractConnexionService implements DataAccessConstants {
    ...
    public void getSync(String repositoryName) throws RepositoryException, IOException {
        ...
        try {
            httpDAOPool.get(0).connectToRepository(repositoryName, repository.getProtocol(),
                    SYNC_FILE_PATH);
    ...
    public void getServerInfo(String repositoryName) throws RepositoryException, IOException {
        ...
        try {
            ...
            serverInfo = httpDAOPool.get(0).downloadSeverInfo(repository.getName(),
                    repository.getProtocol());
        }
        finally {
    ...
    public void getChangelogs(String repositoryName) throws RepositoryException, IOException {
        ...
        try {
            ...
            changelogs = httpDAOPool.get(0).downloadChangelogs(repository.getName(),
                    repository.getProtocol());
        }
        finally {
    ...
    public void getAutoconfig(String repositoryName) throws RepositoryException, IOException {
        ...
        try {
            ...
            autoConfig = httpDAOPool.get(0).downloadAutoconfig(repository.getName(),
                    repository.getProtocol());
        }
        finally {
    ...
    public String determineCompletion(String repositoryName, SyncTreeDirectoryDTO parent)
        ...
        determineFileForComputingCompletion(remotePath, rootDestinationPath, parent, list);
        ...
        if (!noPartialFileTransfer) {
            header = httpDAOPool.get(0).checkPartialFileTransfer(repository.getName(),
                    repository.getProtocol());
        }
        ...
        for (SyncTreeLeafDTO leaf : list) {
            if (httpDAOPool.get(0).isCanceled()) {
                ...
                if (isCompressedPboFilesOnly) {
                    ...
                    double complete = httpDAOPool.get(0).getFileCompletion(leaf.getRemotePath(),
                            leaf.getDestinationPath(), leaf, repository.getProtocol());
    ...
    private void determineFileForComputingCompletion(String rootRemotePath,
            String rootDestinationPath, SyncTreeDirectoryDTO parent, List<SyncTreeLeafDTO> list) {
        ...
        for (SyncTreeNodeDTO node : parent.getList()) {
            if (node.isLeaf()) {
                ...
                determineFileForComputingCompletion(rootRemotePath, rootDestinationPath, directory,
                        list);
            }
        }
    }
                    ...
                    if (!httpDAO.isCanceled()) {
                        ...
                        if (node != null) {
                            Thread t = new Thread(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        ...
                                        File downloadedFile = downloadAddon(httpDAO, node,
                                        ...
                                        if (downloadTimeouterrors.size() > httpDAOPool.size()) {
                                            httpDAO.updateObserverDownloadTooManyTimeoutErrors(
                                                    httpDAOPool.size(), downloadTimeouterrors);
                                        }
                                        else if (downloadErrors.size() > 10) {
                                            httpDAO.updateObserverDownloadTooManyErrors(10,
                                                    downloadErrors);
                                        }
                                        else {
                            ...
                            if (downloadFinished) {
                                ...
                                for (HttpDAO httpDAO : httpDAOPool) {
                                    if (httpDAO.isActiveConnection() && aquireSemaphore()) {
                                        httpDAO.setAcquiredSemaphore(true);
                                        break;
                                    }
                                }
        ...
        for (HttpDAO httpDAO : httpDAOPool) {
            if (!downloadFilesStack.isEmpty()) {// nb files < nb connections
                try {
                    ...
                    httpDAO.connectToRepository(repositoryName, repository.getProtocol(),
                            SYNC_FILE_PATH);
                    httpDAO.disconnect();
                    httpDAO.updateObserverProceed();
                }
    ...
    private File downloadAddon(final HttpDAO httpDAO, final SyncTreeNodeDTO node,
        ...
        return httpDAO.downloadFile(repository.getName(), repository.getProtocol(), remotePath,
    ...
    public boolean upLoadEvents(String repositoryName) throws RepositoryException, IOException {
        ...
        boolean response = httpDAOPool.get(0).uploadEvents(repository.getEvents(),
    ...
    public void getSyncWithRepositoryUploadProtocole(String repositoryName)
        ...
        SyncTreeDirectory syncTreeDirectory = httpDAOPool.get(0).downloadSync(repository.getName(),
        ...
        for (SyncTreeLeaf leaf : this.checkRepositoryFilesList) {
            if (httpDAOPool.get(0).isCanceled()) {
                ...
                for (String fileName : listFilesName) {
                    boolean found = fileExists(repository.getName(), relativePath, fileName,
                            repository.getProtocol());
                    if (!found) {
                        errorsCheckRepository.add(new FileNotFoundException(
                                "File not found on repository: " + relativePath + "/" + fileName));
    ...
    private boolean fileExists(String repositoryName, String relativePath, String fileName,
            AbstractProtocole protocole) throws IOException {

        return httpDAOPool.get(0).fileExists(repositoryName, relativePath, fileName, protocole);
    }
