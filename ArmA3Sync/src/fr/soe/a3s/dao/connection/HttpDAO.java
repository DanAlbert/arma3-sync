...
public class HttpDAO extends AbstractConnexionDAO {
    ...
    private void connect(AbstractProtocole protocole, String relativePathFromRepository)
            throws IOException {
    ...
    private void downloadFile(File file, String relativePath) throws IOException {
    ...
    private void downloadFileWithRecordProgress(File file, String relativePath) throws IOException {
    ...
    private void downloadPartialFileWithRecordProgress(File file, String sha1,
            String relativeFileUrl, String relativeZsyncFileUrl, AbstractProtocole protocole)
            throws IOException {

        try {
            Jazsync.sync(file, sha1, relativeFileUrl, relativeZsyncFileUrl, protocole, this);
        }
        catch (IOException e) {
    ...
    public AutoConfig importAutoConfig(AbstractProtocole protocole) throws IOException {
    ...
    public File downloadFile(String repositoryName, AbstractProtocole protocole, String remotePath,
            String destinationPath, SyncTreeNodeDTO node) throws ConnectException, IOException {
        ...
        if (node.isLeaf()) {
            ...
            if (leaf.getComplete() == 0) {
                ...
                String relativeZsyncFileUrl = remotePath + "/" + node.getName() + ZSYNC_EXTENSION;
                String relativeFileUrl = remotePath + "/" + node.getName();

                String sha1 = leaf.getLocalSHA1();

                try {
                    downloadPartialFileWithRecordProgress(downloadedFile, sha1, relativeFileUrl,
                            relativeZsyncFileUrl, protocole);
                    if (!canceled) {
                        updateObserverDownloadTotalSizeProgress();
                        node.setDownloadStatus(DownloadStatus.DONE);
                    }
    ...
    public double getFileCompletion(String remotePath, String destinationPath,
            SyncTreeNodeDTO node, AbstractProtocole protocole) throws IOException {
        ...
        try {
            complete = Jazsync.getCompletion(targetFile, sha1, relativeZsyncFileUrl, protocole,
                    this);
        }
        catch (IOException e) {
            String coreMessage = "Failed to retreive file " + relativeZsyncFileUrl;
            IOException ioe = transferIOExceptionFactory(coreMessage, e);
            throw ioe;
        }
        catch (HttpException e) {
    ...
    public boolean fileExists(String repositoryName, String relativePath, String fileName,
        ...
        try {
            ...
            String coreMessage = "Failed to connect to repository " + repositoryName + " on url: "
                    + "\n" + protocole.getProtocolType().getPrompt() + protocole.getUrl() + path;
            IOException ioe = transferIOExceptionFactory(coreMessage, e);
            throw ioe;
        }
        finally {
    ...
    public String checkPartialFileTransfer(String repositoryName, AbstractProtocole protocole)
            throws IOException {
    ...
    public boolean uploadEvents(Events events, String repositoryName, AbstractProtocole protocole)
            throws IOException {
