...
public class RepositoryBuilderDAO implements DataAccessConstants, ObservableFilesNumber3,
        ObservableFileSize2 {
    ...
    public void buildRepository(Repository repository) throws Exception {
        ...
        while (iterator.hasNext()) {
            ...
            index = path.toLowerCase().indexOf(File.separator + repositoryName.toLowerCase());
            String folderPath = path.substring(index + repositoryName.length() + 2);
                    ...
                    if (mapOldSync.containsKey(addonName)) {
                        ...
                        if (listleaf.size() != listOldleaf.size()) {
                            ...
                            for (int i = 0; i < listleaf.size(); i++) {
                                if (!listleaf.get(i).getSha1().equals(listOldleaf.get(i).getSha1())) {
                                    changelog.getUpdatedAddons().add(addonName);
                                    break;
                                }
                            }
        ...
        String autoConfigURL = repository.getProtocole().getUrl() + AUTOCONFIG_FILE_PATH;
        if (repository.getProtocole() instanceof Http) {
            if (!repository.getProtocole().getPort().equals("80")) {
                autoConfigURL = getUrlWithPort(repository.getProtocole().getUrl(), repository
                        .getProtocole().getPort());
            }
        }
        ...
        // ServerInfo file
        File serverInfoFile = new File(file.getAbsolutePath() + SERVERINFO_FILE_PATH);
        if (serverInfo != null) {
            ObjectOutputStream fWo = new ObjectOutputStream(new GZIPOutputStream(
        ...
        // AutoConfig file
        File autoConfigFile = new File(file.getAbsolutePath() + AUTOCONFIG_FILE_PATH);
        if (autoConfig != null) {
            ObjectOutputStream fWo = new ObjectOutputStream(new GZIPOutputStream(
        ...
        // Changelogs file
        File changelogsFile = new File(file.getAbsolutePath() + CHANGELOGS_FILE_PATH);
        if (changelogs != null) {
            ObjectOutputStream fWo = new ObjectOutputStream(new GZIPOutputStream(
    ...
    private void getLeafs(SyncTreeDirectory syncTreeDirectory, List<SyncTreeLeaf> listleaf) {
    ...
    private void generateSync(Set<String> excludedFilesFromBuild, final SyncTreeDirectory parent,
        ...
        // exclude .zsync files
                ...
                && !excludedFilesFromBuild.contains(file.getAbsolutePath().toLowerCase())) {
    ...
    public void determineLocalSHA1(SyncTreeNode parent, Repository repository) throws Exception {
        ...
        for (Iterator<String> iter = mapFiles.keySet().iterator(); iter.hasNext();) {
            ...
            if (leaf.getDestinationPath() != null) {
                ...
                if (file.exists()) {
                    ...
                    if (currentFileAttributes != null) {
                        ...
                        long currentLastModified = currentFileAttributes.getLastModified();
                        if (lastModified != currentLastModified || currentSHA1 == null) {
                            compute = true;
                        }
                        else {
                            compute = false;
                    ...
                    if (compute) {
                        Callable<Integer> c = new Callable<Integer>() {
                            ...
                            public Integer call() throws Exception {
                                ...
                                mapFiles.put(path, new FileAttributes(sha1, lastModified));
                                return 0;
                            }
                        };
                        callables.add(c);
    ...
    public void checkRepository(Repository repository) throws RepositoryCheckException {
    ...
    private void check(File file, SyncTreeNode syncTreeNode, Set<String> excludesFiles)
        ...
        if (file.isDirectory()) {
            ...
            for (int i = 0; i < file.listFiles().length; i++) {
                if (!file.listFiles()[i].getName().equals(DataAccessConstants.A3S_FOlDER_NAME)
                        && !file.listFiles()[i].getName().contains(ZSYNC_EXTENSION)
                        && !excludesFiles.contains(file.listFiles()[i].getAbsolutePath())) {
            ...
            for (int i = 0; i < syncTreeDirectory.getList().size(); i++) {
                map.put(syncTreeDirectory.getList().get(i).getName(), syncTreeDirectory.getList()
                        .get(i));
            }
        ...
        try {
            ...
            if (changelogsFile.exists()) {
                ObjectInputStream fRo = new ObjectInputStream(new GZIPInputStream(
                        new FileInputStream(changelogsFile)));
                if (fRo != null) {
    ...
    private Events readEvents(File file) {
        ...
        try {
            ...
            if (eventsFile.exists()) {
                ObjectInputStream fRo = new ObjectInputStream(new GZIPInputStream(
                        new FileInputStream(eventsFile)));
                if (fRo != null) {
    ...
    private ServerInfo readServerInfo(File file) {
        ...
        try {
            ...
            if (serverInfoFile.exists()) {
                ObjectInputStream fRo = new ObjectInputStream(new GZIPInputStream(
                        new FileInputStream(serverInfoFile)));
    ...
    public void updateFileSizeObserver2() {
        observerFileSize2.update((int) (this.cumulativeFileSize * 100 / totalFilesSize));
    }
